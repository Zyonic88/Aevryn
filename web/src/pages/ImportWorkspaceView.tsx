...
export function ImportWorkspaceView({ project }: { project: ProjectSummary }) {
  ...
  const [isImportIdAutoGenerated, setIsImportIdAutoGenerated] = useState(true);
  ...
  const createImport = useMutation({
    mutationFn: ({
      ...
      return apiClient.createStoryImport(
        project.id,
        storyId,
        { ...payload, import_id: importId, now },
        requireSessionToken(session),
        now,
      );
    },
  ...
  async function saveImportMetadata() {
    ...
    try {
      ...
      const importIdForSave = nextImportIdForSave({
        ...
        existingImports: storyId === activeStoryId ? (importsQuery.data?.imports ?? []) : [],
        isAutoGenerated: isImportIdAutoGenerated,
      });
    ...
    try {
      if (files.length === 1) {
        ...
        if (generatedSourceId) {
          ...
          setIsImportIdAutoGenerated(true);
        }
...
function createImportId(sourceId: string): string {
  return `import_${machineSuffix(sourceId)}_${machineSuffix(new Date().toISOString())}_${randomImportSuffix()}`;
}

function nextImportIdForSave({
  currentImportId,
  existingImports,
  isAutoGenerated,
  sourceId,
  ...
  existingImports: ImportRecord[];
  isAutoGenerated: boolean;
  sourceId: string;
}): string {
  if (!isAutoGenerated) {
    return currentImportId.trim();
  }
  const existingImportIds = new Set(existingImports.map((record) => record.import_id));
  let candidate = createImportId(sourceId);
  while (existingImportIds.has(candidate)) {
    candidate = createImportId(sourceId);
  }
  return candidate;
}

function randomImportSuffix(): string {
  if (typeof crypto !== "undefined" && typeof crypto.randomUUID === "function") {
    return machineSuffix(crypto.randomUUID()).slice(0, 12);
  }
...
function isDuplicateImportError(error: unknown): boolean {
  return (
    ...
    (error.status === 409 || error.code === "duplicate_record") &&
    error.message.startsWith("Duplicate import:")
  );
}
