  ...
  character_profiles: [
    {
      ...
      appearance: { title: "Appearance", items: ["Unknown"] },
      ...
      first_appearance: { title: "First Appearance", items: ["Unknown"] },
      latest_appearance: { title: "Latest Appearance", items: ["Unknown"] },
      timeline_history: { title: "Timeline History", items: ["Unknown"] },
      evidence_references: { title: "Evidence References", items: ["Unknown"] },
...
describe("AevrynApiClient", () => {
  ...
  it("sends authenticated requests for project stories", async () => {
    ...
    for (const [input, init] of fetchMock.mock.calls.slice(0, 2)) {
    ...
    expect(fetchMock.mock.calls[1][1].method).toBe("POST");
    expect(fetchMock.mock.calls[2][0]).toBe(
      `https://api.aevryn.ai${API_PATHS.projects}/project_alpha/stories/story_alpha`,
    );
  ...
  it("sends authenticated requests for project runs", async () => {
    ...
    await expect(
      client.processWorkerJobs({
        started_at: "2026-06-27T00:00:01.000Z",
        finished_at: "2026-06-27T00:00:02.000Z",
        max_jobs: 10,
      }),
    ).resolves.toEqual({ claimed_jobs: 1, succeeded_jobs: 1, failed_jobs: 0 });
