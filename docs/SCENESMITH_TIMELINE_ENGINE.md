...
The Timeline Engine tells SceneSmith when that truth is valid.
...
With timeline authority, SceneSmith can answer:
...
SceneSmith's value depends on continuity at a specific point in the story.

A fact is not enough by itself.

SceneSmith also needs to know:
