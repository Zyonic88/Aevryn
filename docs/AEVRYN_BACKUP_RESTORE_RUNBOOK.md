...
* `AEVRYN_RESTORE_DRILL_BEARER_TOKEN` for the restore-test owner, unless
  `--prompt-session-tokens` or `--clipboard-session-tokens` is used
...
On Windows terminals where hidden prompts block paste, use
`--clipboard-session-tokens` instead. It asks the operator to copy each token to
the clipboard one at a time and reads it without printing the value.
