...
Its job is to determine whether Aevryn can import from a web source, collect source metadata, discover chapter links when allowed, and preserve attribution for evidence references.
...
Aevryn eventually needs a safe way to help users import those sources without violating site rules, bypassing access restrictions, or republishing copyrighted text.
...
It never presents scraped copyrighted text as Aevryn-owned content.
...
The Export Engine may include source attribution and evidence references, but it must not republish source chapters as Aevryn-owned content.

The Project Manager may coordinate Web Import as one intake option.

## V1 Boundary

For V1, Aevryn should prioritize:
...
Never present scraped copyrighted text as Aevryn-owned content.
