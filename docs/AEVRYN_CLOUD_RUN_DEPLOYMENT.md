...
`AEVRYN_SUPABASE_JWT_ALGORITHM=rs256` is preferred for public beta and requires
`AEVRYN_SUPABASE_JWKS_URL`. If the Supabase project issues HS256 access tokens,
set `AEVRYN_SUPABASE_JWT_ALGORITHM=hs256` and store `AEVRYN_SUPABASE_JWT_SECRET`
in the deployment secret manager. Never place the JWT secret in frontend
