...
Managed-identity browser flow and creator workflow smoke are still required.
...
```text
Public beta: Blocked
Reason: Cloud Run API health smoke, custom-domain API health smoke, and hosted frontend/API header smoke passed, but managed-identity and creator workflow smoke have not passed.
```
